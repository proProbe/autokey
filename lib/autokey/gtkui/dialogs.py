...
class HotkeySettingsDialog(DialogBase):
    ...
    def __init__(self, parent, configManager, closure):
        ...
        self.capslockButton= builder.get_object("capslockButton")
    ...
    def load(self, item):
        ...
        if model.TriggerMode.HOTKEY in item.modes:
            ...
            self.capslockButton.set_active(iomediator.Key.META in item.modifiers)
    ...
    def set_key(self, key, modifiers=[]):
        ...
        self.capslockButton.set_active(iomediator.Key.CAPSLOCK in modifiers)
    ...
    def build_modifiers(self):
        ...
        if self.capslockButton.get_active():
...
class GlobalHotkeyDialog(HotkeySettingsDialog):
    ...
    def load(self, item):
        ...
        if item.enabled:
            ...
            self.hyperButton.set_active(iomediator.Key.HYPER in item.modifiers)
            self.metaButton.set_active(iomediator.Key.META in item.modifiers)
            self.capslockButton.set_active(iomediator.Key.CAPSLOCK in item.modifiers)
